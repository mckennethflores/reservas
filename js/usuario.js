...
function init() {
	...
	$.post(base_url+"/ajax/login.php?op=selectTiendas", function (r) {
		$("#idtienda").html(r);
	});

	$.post( base_url+"/ajax/usuario.php?op=permisos&id=", function (r) {
		$("#permisos").html(r);
	});

	//Cargamos sexo cbo
	$.post(base_url+"/ajax/usuario.php?op=selectSexousuario", function (r) {
		$("#sexousuario").html(r);
	});
	//Cargamos PERFIL CLIENTE, EMPLEADO
	$.post(base_url+"/ajax/usuario.php?op=selectPerfilUsuario", function (r) {
		$("#idperfil").html(r);
	});
}
...
function listar() {
	tabla = $('#tbllistado').dataTable({
		...
		"ajax": {
			url: base_url +'/ajax/usuario.php?op=listar',
			type: "get",
			dataType: "json",
			error: function (e) {
...
function guardaryeditar(e) {
	...
	$.ajax({
		url: base_url+"/ajax/usuario.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(idusuario) {
	$.post(base_url+"/ajax/usuario.php?op=mostrar", {
		idusuario: idusuario
...
/* 		$("#idtienda").val(data.idtienda);
	...
	$.post(base_url+"/ajax/usuario.php?op=permisos&id=" + idusuario, function (r) {
		$("#permisos").html(r);
	});
}
...
function desactivar(idusuario) {
	bootbox.confirm("¿Está Seguro de desactivar el usuario?", function (result) {
		if (result) {
			$.post(base_url+"/ajax/usuario.php?op=desactivar", {
				idusuario: idusuario
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
...
function activar(idusuario) {
	bootbox.confirm("¿Está Seguro de activar el usuario?", function (result) {
		if (result) {
			$.post(base_url+"/ajax/usuario.php?op=activar", {
				idusuario: idusuario
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
